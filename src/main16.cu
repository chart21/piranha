...
#ifdef ONEPC
    NeuralNetwork<uint16_t, OPC> net(&nn_config, piranha_config["nn_seed"]);
#else
#ifdef TWOPC
    NeuralNetwork<uint16_t, TPC> net(&nn_config, piranha_config["nn_seed"]);
#else
#ifdef FOURPC
    NeuralNetwork<uint16_t, FPC> net(&nn_config, piranha_config["nn_seed"]);
#else
    NeuralNetwork<uint16_t, RSS> net(&nn_config, piranha_config["nn_seed"]);
#endif
#endif
#endif
        ...
        for (int i = 0; i < numIterations; i++) {
            ...
            if (piranha_config["iteration_snapshots"]) {
                ...
                if (piranha_config["test_iteration_snapshots"]) {
                    NeuralNetwork<uint16_t, OPC> test_net(config, 0);
